...
class BacktrackingSearch(BaseSolver):
    ...
    def _check_consistency(self, clues: np.ndarray, grid: np.ndarray, row_update: int, col_update: int) -> bool:
    ...
    def _single_run(self, clues: np.ndarray, prev: np.ndarray, num_steps: int) -> np.ndarray:
        ...
        try:
            solved = self._rec_search(clues, known_grid, steps, initial=True, max_steps=num_steps)
