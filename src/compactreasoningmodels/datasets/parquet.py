...
class ParquetPuzzleDataset(BaseDataset):
    ...
    def _prepare_tensors(self, target_shape: tuple[int, ...] | None):
        df = self._df

        grid_heights = np.asarray(df["grid_height"].values)
        grid_widths = np.asarray(df["grid_width"].values)
        ...
        for i, sol in enumerate(solutions):
            if isinstance(sol, np.ndarray):
                sol_array = np.stack(
                    sol.tolist() if sol.dtype == object else [r for r in sol]
                ).astype(np.float32)
            else:
    ...
    def intermediate_solutions(self) -> list[np.ndarray]:
        ...
        return [
            np.array(g.tolist() if hasattr(g, "tolist") else g, dtype=np.float64) for g in grids
        ]
    ...
    def requires_search(self) -> np.ndarray:
        return np.asarray(self._df["requires_search"].values)

    @property
    def grid_density(self) -> np.ndarray:
        return np.asarray(self._df["grid_density"].values)

    @property
    def steps(self) -> np.ndarray:
        return np.asarray(self._df["steps"].values)

    @property
    def one_step_rounding(self) -> np.ndarray:
        return np.asarray(self._df["one_step_rounding"].values)

    @property
    def puzzle_ids(self) -> np.ndarray:
        return np.asarray(self._df["puzzle_id"].values)
