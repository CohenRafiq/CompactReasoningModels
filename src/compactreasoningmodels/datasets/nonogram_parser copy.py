...
class NonogramDataset:
    ...
    def __init__(self, data: str | Path | t.Dataset, 
        ...
        X_raw, self.y, self.meta = self.parse(data)
        if padding == "positional":
            self.X, self.padding_mask = self._pad_positional(X_raw, self.meta["shape"])
        elif padding == "sequence":
            self.X, self.padding_mask = self._pad_sequence(X_raw, self.meta["shape"])
        else:
    ...
    def _pad_sequence(self, clues: list[t.Clues], max_dimension: int) -> tuple[list[t.Clues], list[list[float]]]:
        max_runs_in_clue = (max_dimension + 1) // 2 # worst case: alternating 1s and 0s 
        max_padding = 2 * max_dimension * max_runs_in_clue
        ...
        seq = [[[[[val, i, j, k] 
                  ...
                  for puzzle in clues]
        flat_seq = [item for puzzle in seq for group in puzzle for line in group for item in line]
        padded_seq = flat_seq + [[0, 0, 0, 0]] * (max_padding - len(flat_seq))
        padding_mask = [0.0] * len(flat_seq) + [-torch.inf] * (max_padding - len(flat_seq))
        return (
