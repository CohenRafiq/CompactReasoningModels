...
class BaseExperiment(ABC):
    ...
    def generate_heatmaps(self, num_samples: int = 100, initial_grid: str = "random"):
        for i, (input_tensor, target_grid) in enumerate(self.dataloader):
            ...
            for solver_name, num_steps in self.next_solver():
                solver_class = self.solvers[solver_name]
                solver_instance = solver_class(input_tensor, self.target_shape, initial_grid=initial_grid_array)
                heatmap = solver_instance.heatmap(num_steps=num_steps)
                target_grid = target_grid.squeeze().cpu().numpy().reshape(self.target_shape)
                yield (solver_name, num_steps, heatmap, target_grid)

    @abstractmethod
    def next_solver(self) -> Generator[tuple[str, int], None, None]:
