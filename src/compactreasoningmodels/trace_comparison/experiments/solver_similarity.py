...
from compactreasoningmodels.trace_comparison.experiments import BaseExperiment

class FullDatasetSimilarityExperiment(BaseExperiment):
    def __init__(self, blocks: list, data: torch.Tensor | np.ndarray, name: str = "full_dataset_similarity_experiment"):
    ...
    def display_correlation_matrix(self):
        ...
        plt.xticks(ticks=np.arange(self.correlation_matrix.shape[0]), labels=np.arange(self.correlation_matrix.shape[0]))
        plt.yticks(ticks=np.arange(self.correlation_matrix.shape[0]), labels=np.arange(self.correlation_matrix.shape[0]))
        plt.show()