...
class BaseExperiment(ABC):
    ...
    def __init__(self, blocks: List[Block], data: torch.Tensor | np.ndarray,  name: str = "experiment"):
    ...
    def validate_shapes(self, input_shape: Tuple[int, ...]) -> None:
        ...
        if len(input_shape) != 4:
            raise ShapeError(f"Input shape must be 4D (solvers, batch, steps, grid), got {len(input_shape)}D")
