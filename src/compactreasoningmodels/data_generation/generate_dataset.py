...
def generate_puzzle(clue_generator, constraint_propagator, rng, max_attempts=10000):
    ...
    grid_density = sum(x for inner in row_clues for x in inner) / (
        clue_generator.h * clue_generator.w
    )
...
def _generate_one(max_attempts, safety_cap=100_000):
    ...
    while True:
        ...
        puzzle = generate_puzzle(
            _worker_clue_gen, _worker_prop, _worker_rng, max_attempts=max_attempts
        )
...
def main(
    ...
    if n_workers == 1:
        ...
        result_stream: Iterable[tuple[dict, int]] = (
            _generate_one(max_attempts) for _ in range(num_samples)
        )
        pool_ctx = None
    else:
        pool_ctx = mp.Pool(
    ...
    try:
        for puzzle, attempts in result_stream:
            ...
            if len(batch_records) >= batch_size:
                ...
                print(f"Written {generated}/{num_samples} puzzles...")
    except Exception as e:
        print(f"\nException after {generated} puzzles: {e}")
        _flush_batch(writer, batch_records, schema)
        raise
    finally:
        _flush_batch(writer, batch_records, schema)
...
def puzzles_to_arrays(puzzles):
    """
    Convert a list of puzzle dicts to (X, y) numpy arrays.

    X shape: (n, 2, grid_size, max_clue_len)
    y shape: (n, h, w)
    """
    n = len(puzzles)
    h = len(puzzles[0]["rows"])
    w = len(puzzles[0]["cols"])
    max_clue_len = max(max((len(r) for r in p["rows"]), default=1) for p in puzzles)
    max_clue_len = max(
        max_clue_len, max(max((len(c) for c in p["cols"]), default=1) for p in puzzles)
    )

    X = np.zeros((n, 2, max(h, w), max_clue_len), dtype=np.float32)
    y = np.zeros((n, h, w), dtype=np.uint8)

    for i, p in enumerate(puzzles):
        for r_idx, clue in enumerate(p["rows"]):
            X[i, 0, r_idx, : len(clue)] = clue
        for c_idx, clue in enumerate(p["cols"]):
            X[i, 1, c_idx, : len(clue)] = clue
        for r_idx, row in enumerate(p["solution"]):
            for c_idx, cell in enumerate(row):
                y[i, r_idx, c_idx] = 1 if cell == 1 else 0

    return X, y


def save_dataset(easy, hard, prefix="nonogram"):
    easy_X, easy_y = puzzles_to_arrays(easy)
    hard_X, hard_y = puzzles_to_arrays(hard)

    paths = {
        f"{prefix}_easy_X.npy": easy_X,
        f"{prefix}_easy_y.npy": easy_y,
        f"{prefix}_hard_X.npy": hard_X,
        f"{prefix}_hard_y.npy": hard_y,
    }
    for path, arr in paths.items():
        np.save(path, arr)
        mb = os.path.getsize(path) / 1e6
        print(f"Saved -> {path}  shape={arr.shape}  dtype={arr.dtype}  ({mb:.1f} MB)")
...
if __name__ == "__main__":
    ...
    main(
        ...
        density=0.5,
        chunksize=100,
        parquet_path="data/raw/nng_test.parquet",
    )
