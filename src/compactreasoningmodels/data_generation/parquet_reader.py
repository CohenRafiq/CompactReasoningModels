...
class ParquetReader:
    def __init__(
        self, parquet_file: str, random_seed: int = 42, target_shape: tuple[int, int] | None = None
    ):
        ...
        df["row_clues"] = pd.Series(
            [
                self.convert_clue(rc, max_len=(gw + 1) // 2)
                for rc, gw in zip(df["row_clues"], df["grid_width"], strict=True)
            ],
            dtype=object,
        )
        df["col_clues"] = pd.Series(
            [
                self.convert_clue(cc, max_len=(gh + 1) // 2)
                for cc, gh in zip(df["col_clues"], df["grid_height"], strict=True)
            ],
            dtype=object,
        )
        ...
        X = torch.from_numpy(
            np.stack(
                [
                    np.stack([rc, cc])
                    for rc, cc in zip(df["row_clues"], df["col_clues"], strict=True)
                ],
                axis=0,
            )
        )
        ...
        y = torch.stack(
            [
                ...
                if hasattr(sol, "dtype") and sol.dtype != object
                else torch.from_numpy(np.array(sol.tolist(), dtype=np.int8)).float()
                for sol in df["solution"]
            ]
        )

        df["X"] = pd.Series(list(X), dtype=object)
        df["y"] = pd.Series(list(y), dtype=object)
    ...
    def convert_clue(self, clue, max_len: int) -> np.ndarray:
        ...
        return np.array(
            [list(inner) + [0] * max(0, max_len - len(inner)) for inner in clue], dtype=np.float32
        )
    ...
    def _apply_query(self, query: str | pd.Series | None) -> pd.DataFrame:
        if query is None:
            ...
            mask = (
                query.reindex(self.dataframe.index, fill_value=False)
    ...
    def _split_indices(
        self, n: int, train_ratio: float, random_seed: int | None
    ...
    def get_solution_steps(self, idx: int) -> list[tuple[np.ndarray, str]]:
        ...
        return list(zip(grids, methods, strict=True))
