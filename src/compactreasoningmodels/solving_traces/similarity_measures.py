...
class SimilarityMeasure(ABC):
    ...
    def __call__(self, grid1: np.ndarray, grid2: np.ndarray) -> Union[float, np.ndarray]:
        g1 = np.asarray(grid1)
        g2 = np.asarray(grid2)
    ...
    @abstractmethod
    def compute_similarity(self, grid1: np.ndarray, grid2: np.ndarray) -> Union[float, np.ndarray]:
        ...
