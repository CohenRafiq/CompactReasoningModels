...
class NNGSupervisedTrainer(BaseTrainer):
    ...
    def _print_epoch(self, epoch: int, metrics: Dict[str, float]) -> None:
        msg = (
            f"Epoch {epoch:02d}: "
            f"Train Loss={metrics.get('train_loss', 0.0):.4f}, Train Acc={metrics.get('train_accuracy', 0.0):.4f} | "
            f"Test Loss={metrics.get('test_loss', 0.0):.4f}, Test Acc={metrics.get('test_accuracy', 0.0):.4f}"
        )
        if "train_abstain_rate" in metrics:
