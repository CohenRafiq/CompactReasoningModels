...
from typing import Tuple, Dict, Optional, Union, List, Set
...
class ConstraintPropagator:
    ...
    def __init__(
        self,
        prior_cache: Optional[Dict[Tuple[Tuple[int, ...], int], Optional[np.ndarray]]] = None,
        local_cache_maxsize: int = 5000,
    ):
        # Tier 1: shared, high hit-rate, unbounded (state space is small).
        # Pass in a dict built/broadcast from the main process to share
        # warm entries across workers.
        self._prior_cache = prior_cache if prior_cache is not None else {}

        # Tier 2: local, low hit-rate, must be bounded (LRU).
        self._prob_cache: "OrderedDict[Tuple, Optional[np.ndarray]]" = OrderedDict()
    ...
    def clear_caches(self, prior: bool = False, local: bool = True) -> None:
        """Drop cached probability vectors. By default only clears the
        low-value local (tier 2) cache; the shared prior cache (tier 1)
        is left intact since it's cheap and worth keeping."""
        if local:
            self._prob_cache.clear()
        if prior:
            self._prior_cache.clear()

    def prior_cache_snapshot(self) -> Dict[Tuple[Tuple[int, ...], int], Optional[np.ndarray]]:
        """Return a plain dict copy of the tier-1 cache, e.g. to hand to
        a multiprocessing pool initializer."""
    ...
    def solve_grid(
        ...
        prev_row_grid: Optional[np.ndarray] = None,
        prev_col_grid: Optional[np.ndarray] = None,
        dirty_rows: Optional[Set[int]] = None,
        dirty_cols: Optional[Set[int]] = None,
    ) -> Union[
        Optional[np.ndarray],
    ...
    def row_probabilities(
        ...
        # ---- Tier 1: fully-unknown case, shared/high hit-rate ----------
        if is_fully_unknown:
        ...
        # ---- Tier 2: partially-known case, local/low hit-rate ----------
        local_key = (blocks, length, tuple(known_arr))
        cached = self._prob_cache.get(local_key, _MISS)
    ...
    # ------------------------------------------------------------------
    # DP core (unchanged logic, just extracted so both cache tiers
    # can share it)
    # ------------------------------------------------------------------
    def _compute_probabilities(
        ...
        # --- O(1) range helper: any forced-empty cells in [i, i+b)? ---
        ...
        # ==================================================================
        # 3. PER-CELL COUNTS via difference array
        # ==================================================================
        diff = [0] * (length + 1)
    ...
    def _normalize_known(
        ...
        None (no known array at all) is treated identically to an
        all-unknown tuple, so both hit tier 1 consistently."""
        if known is None:
            return [-1] * length
        if len(known) != length:
            raise ValueError(
